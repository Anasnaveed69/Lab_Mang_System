...
void AcademicOfficerUI::addSection() {
  ...
  Section section(sectionId, labName, courseCode, instructorId, roomId,
                  semester);
  while (true) {
...
void AcademicOfficerUI::viewMakeupRequests() {
  ...
  if (!hasApproved)
    cout << "No approved requests history." << endl;
}
...
void AcademicOfficerUI::approveMakeupRequest() {
  ...
  cout << "\n=== Makeup Request Details ===" << endl;
  cout << "Request ID: " << request->getRequestId() << endl;
  cout << "Section ID: " << request->getSectionId() << endl;
  cout << "Missed Date: " << request->getRequestedDate() << endl;
  cout << "Reason: " << request->getReason() << endl;
  ...
  // Try to find original session to copy timing from
  // Search for session matching BOTH section ID and the missed date
  Date missedDate = request->getRequestedDate();
  vector<LabSession> allSessions = StorageManager::loadSessions();
  LabSession *originalSession = nullptr;

  cout << "\nSearching for original session:" << endl;
  cout << "  Section ID: " << request->getSectionId() << endl;
  cout << "  Missed Date: " << missedDate << endl;

  for (auto &s : allSessions) {
    // Check both section ID and date match (using Date's == operator)
    if (s.getSectionId() == request->getSectionId() &&
        s.getDate() == missedDate) {
      originalSession = &s;
      cout << "  ✓ Found original session: " << s.getSessionId() << endl;
      break;
    }
  }

  Time expStart, expEnd;
  if (originalSession == nullptr) {
    // Show available sessions for this section to help debug
    cout << "\nNo original session found for the missed date: " << missedDate
         << endl;
    vector<LabSession> sectionSessions =
        StorageManager::findSessionsBySection(request->getSectionId());
    if (sectionSessions.empty()) {
      cout << "  No sessions found for this section at all." << endl;
    } else {
      cout << "  Available sessions for section " << request->getSectionId()
           << ":" << endl;
      for (const auto &s : sectionSessions) {
        cout << "    - " << s.getSessionId() << " on " << s.getDate() << endl;
      }
    }
    cout << "\nThis is normal if the session was never scheduled or was "
            "cancelled."
         << endl;
    cout << "Please enter the expected timing for the makeup session:" << endl;
    expStart = InputUtil::readTime("Enter Expected Start Time");
    expEnd = InputUtil::readTime("Enter Expected End Time");

    // Validate: end time should be after start time
    if (expEnd.toMinutes() <= expStart.toMinutes()) {
      cout << "Error: End time must be after start time!" << endl;
      return;
    }
  } else {
    // Use timing from original session
    expStart = originalSession->getTiming().getExpectedStart();
    expEnd = originalSession->getTiming().getExpectedEnd();
    cout << "\n✓ Found original session. Using its timing: " << expStart
         << " - " << expEnd << endl;
  }

  string sessionId = "MAKEUP_" + requestId;
  LabSession makeupSession(sessionId, request->getSectionId(), makeupDate,
                           expStart, expEnd);
